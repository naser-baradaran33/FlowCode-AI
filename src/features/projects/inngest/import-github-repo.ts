...
export const importGithubRepo = inngest.createFunction(
  {
    ...
    // @ts-expect-error runtime accepts event, but type definitions do not
  ...
  async ({ event, step }: { event: any; step: any }) => {
    ...
    const tree = await step.run(
      "fetch-repo-tree",
      async (): Promise<{ tree: GitTreeEntry[] }> => {
        try {
    ...
    const folders = tree.tree
      .filter((item: GitTreeEntry) => item.type === "tree" && item.path)
      .sort((a: GitTreeEntry, b: GitTreeEntry) => {
    ...
    const allFiles = tree.tree.filter(
      (item: GitTreeEntry) => item.type === "blob" && item.path && item.sha
    );
